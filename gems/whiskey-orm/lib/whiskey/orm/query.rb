...
module Whiskey
  module ORM
    # Optional query module for Glass objects
    # Provides chainable query DSL for filtering, ordering, limits
    module Query
          ...
          unless @conditions.empty?
            ...
            @conditions.each do |key, value|
              if key == :raw
                ...
                if value.is_a?(Array)
                  where_parts << "#{key} IN (#{value.map { |v| quote_value(v) }.join(', ')})"
                else
                  where_parts << "#{key} = #{quote_value(value)}"
                end
              end
            end
