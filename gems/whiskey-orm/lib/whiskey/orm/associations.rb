...
module Whiskey
  module ORM
    # Optional associations module for Glass objects
    # Provides declarative association methods (has_one, has_many, belongs_to)
    module Associations
      ...
      module InstanceMethods
        ...
        def set_association(name, value)
          ...
          if association[:type] == :belongs_to && value
            if value.respond_to?(:attributes) && value.attributes[association[:primary_key]]
          ...
          if primary_key_value
            collection.each do |item|
              if item.respond_to?(:attributes)
                item.attributes[association[:foreign_key]] = primary_key_value
              end
            end
          end
        end
